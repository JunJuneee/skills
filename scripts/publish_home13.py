...
SOURCE = Path('/Users/jun/Documents/블로그 포스팅/8월1일_홈쇼핑_시간대별_게시글/8월1일_13시_홈쇼핑_편성표.md')
ASSETS = Path('/private/tmp/home13')
TITLE = '8월 1일 13시 홈쇼핑 편성표｜대표상품·함께방송 상품 링크'
LABELS = ['홈쇼핑', '홈쇼핑편성표', '13시홈쇼핑', '오늘의홈쇼핑', '홈쇼핑방송', '홈쇼핑상품', '홈쇼핑할인']
...
def markdown_to_html(markdown):
    ...
    while i < len(lines):
        ...
        if line.startswith('### '):
            ...
            out.append(f'<h3 id="{ident}">{inline(line[4:])}</h3>')
            i += 1
            continue
        if line.startswith('## '):
            ident = re.sub(r'[^a-z0-9가-힣]+', '-', line[3:].lower()).strip('-')
            out.append(f'<h2 id="{ident}">{inline(line[3:])}</h2>')
            i += 1
            continue
        if line.startswith('# '):
            i += 1
...
marker = '## 13시 홈쇼핑 대표상품 편성표\n\n'
cards = '\n\n'.join([
    image('card_02.jpg', '8월 1일 13시 홈쇼핑 대표상품 카드 1'),
    image('card_03.jpg', '8월 1일 13시 홈쇼핑 대표상품 카드 2'),
    image('card_04.jpg', '8월 1일 13시 홈쇼핑 대표상품 카드 3'),
    image('card_05.jpg', '8월 1일 13시 홈쇼핑 대표상품 카드 4'),
])
if marker not in source:
    raise RuntimeError('representative product section marker not found')
source = source.replace(marker, marker + '대표상품은 해당 방송에서 중심으로 소개되는 구성입니다.\n\n@@REPRESENTATIVE_CARDS@@\n\n', 1)
source = source.replace(
    '## 함께 보면 좋은 글\n\n- 오늘의 홈쇼핑 편성표 채널별 확인 방법\n- 홈쇼핑 카드 할인과 무이자 할부 확인 방법\n- 홈쇼핑 건강식품 구매 전 확인사항\n- 홈쇼핑 여행 상품 예약 전 체크리스트',
    '''## 함께 보면 좋은 글

- [8월 1일 12시 홈쇼핑 편성표｜대표상품·함께방송 상품 링크](https://betterpickguide.blogspot.com/2026/08/8-1-12.html)
- [8월 1일 11시 홈쇼핑 편성표｜대표상품·함께방송 상품 링크](https://betterpickguide.blogspot.com/2026/08/8-1-11.html)
...
content += image('card_01.jpg', '8월 1일 13시 홈쇼핑 편성표 표지')
content += markdown_to_html(source)
content = content.replace('<p>@@REPRESENTATIVE_CARDS@@</p>', cards)
content += '<h2>홈쇼핑 편성표 더 보기</h2>'
content += image('cta.jpg', '8월 1일 13시 홈쇼핑 편성표 더 보기')
content += '</article>'

expected_alts = re.findall(r'<img[^>]+alt="([^"]+)"', content)
secret = json.loads(Path('/Users/jun/Downloads/client_secret.json').read_text())
oauth = secret.get('web') or secret.get('installed')
tokens = json.loads((ROOT / 'mcp-blogspot-posting/.blogger-tokens.json').read_text())
cache = json.loads((ROOT / 'mcp-blogspot-posting/.blog_id_cache.json').read_text())
...
if matches:
    ...
    method = 'POST'
with urlopen(Request(endpoint, data=payload, method=method, headers=headers), timeout=120) as response:
